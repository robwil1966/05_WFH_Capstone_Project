...
def create_app(test_config=None):
  ...
  def retrieve_walkers(payload):
    try:
      ...
      walkers = list(map(Walker.format, Walker.query.order_by(Walker.id).all()))
      print(walkers)
  ...
  def edit_walker(payload, walker_id):
      body = request.get_json()
      walker_to_edit = Walker.query.filter(Walker.id == walker_id).one_or_none()
  ...
  def retrieve_groups():
    try:
      ...
      groups = list(map(Group.format, Group.query.order_by(Group.group_name).all()))
      print(groups)
  ...
  def get_routes():
    try:
      ...
      routes = list(map(Route.format, Route.query.order_by(Route.id).all()))
      print(routes)
  ...
  def create_route(payload):
    ...
    try:
      ...
      routes = list(map(Route.format, Route.query.order_by(Route.route_name).all()))
      '''current_questions = paginate_questions(request, selection)'''
  ...
  def get_events():
    try:
      ...
      events = list(map(Event.format, Event.query.order_by(Event.id).all()))
      print(events)
  ...
  def search_walker():
    ...
    try:
      ...
      walkers = Walker.query.filter(Walker.first_name.ilike('%' + search + '%')).all()
